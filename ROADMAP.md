...
| **pupil-tracking** [built] | a per-frame pupil detector run across a video with temporal consistency (movement + size gate, gap interpolation), so blinks and distractors don't break the pupil-size trace | the per-frame detector alone · trained pose models (DeepLabCut) | mixed (consent) | the per-frame detector is Sonja Nevelchuk's (used with permission); the tracking tool is the contribution |
