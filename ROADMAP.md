...
| **dff-baseline** [built] | a rolling MEDIAN baseline, left unclipped, so ΔF/F sits symmetric around zero instead of biased up | a lower-envelope / low-percentile baseline · a polynomial lower-envelope (peakutils) | hers | the unclipped median-baseline mode is the contribution; the comparison figure (median centred on 0 vs envelope biased up) is the tile |
| **pupil-tracking** [built] | the pupil measured by fitting its BORDER (rays to the half-drop, a circle through the crossings whose fall held) instead of thresholding an area, with a refusal where the border is not visible, plus the temporal layer (anchoring, re-acquisition, gap interpolation, transient-jump refusal) | a bright-blob / dark-blob area detector · trained pose models (DeepLabCut) | hers | the border fit, its guards and the calibration are the contribution; an earlier version of the tile ran the temporal layer on Sonja Nevelchuk's dark-pupil detector (with permission) and no longer does |
| **osi-stats** [built] | orientation selectivity (OSI) called with a z-scored trial-shuffle test + Benjamini-Hochberg FDR across the population, so the OSI point estimate's positive bias doesn't leak untuned boutons into the "tuned" count | a bare OSI threshold · per-neuron significance with no population correction | wrapper | OSI, shuffle test, and BH-FDR are standard; the rigorous population call is the contribution |
